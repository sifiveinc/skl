 ...
 */
SKL_FUNC_PRIVATE void skl_pack_tile_load_e8_e8_xsfmmbase(size_t tm, size_t tn,
                                                         const uint8_t *src,
  ...
  const uint8_t *src_end = src + tm * rs;

  __asm__ volatile("sf.vsettnt x0, %[tn], e8, w1\n"
                   ...
                   : [src_end] "r"(src_end), [kRowInc] "rI"(kRowInc),
                     [rs] "r"(rs * sizeof(uint8_t)), [tm] "r"(tm), [tn] "r"(tn)
                   : "vtype", "vl", "memory");
}
 ...
 */
SKL_FUNC_PRIVATE void skl_pack_tile_store_pad_bottom_e8_e8_xsfmmbase(
    size_t tm, size_t tss, size_t m0, size_t n0,
    uint8_t *dst, // NOLINT(readability-non-const-parameter)
    size_t rs, uint8_t pad) __riscv_in("xsfmm") {
  if (m0 == 0 || n0 == 0) {
    return;
  }

  vuint8m2_t pad_vec = __riscv_vmv_v_x_u8m2(pad, n0);
  const size_t kRowInc = 1;
  const uint8_t *dst_store_end = dst + tm * rs;
  const uint8_t *dst_pad_end = dst + m0 * rs;

  __asm__ volatile(
      "sf.vsettnt x0, %[n0], e8, w1\n"
      ...
      : [dst_store_end] "r"(dst_store_end), [dst_pad_end] "r"(dst_pad_end),
        ...
        [rs] "r"(rs * sizeof(uint8_t)), [m0] "r"(m0), [n0] "r"(n0), [tm] "r"(tm)
      : "vtype", "vl", "memory");
}

/*
 * Equivalent to:
 * skl_pack_tile_store_pad_bottom_e8_e8_xsfmmbase(tm_store, tss_store, m0,
                                                    n0, dst, rsdst, pad);
 * skl_pack_tile_load_e8_e8_xsfmmbase(tm_load, tn_load, src, rssrc,
                                        tss_load);
 *
 * This kernel overlaps store and load execution for improved performance over
 * storing tss_store first and then loading src.
 */
SKL_FUNC_PRIVATE void skl_pack_tile_store_load_e8_e8_xsfmmbase(
    size_t tm_store, size_t tss_store, size_t m0, size_t n0,
    uint8_t *dst, // NOLINT(readability-non-const-parameter)
    size_t rsdst, size_t tm_load, size_t tn_load, const uint8_t *src,
    size_t rssrc, size_t tss_load, uint8_t pad) __riscv_inout("xsfmm") {
  ...
  const uint8_t *src_end = src + tm_load * rssrc;
  const uint8_t *dst_store_load_end = dst + k0 * rsdst;
  const uint8_t *dst_pad_load_end = dst + k1 * rsdst;
  const uint8_t *dst_store_end = dst + tm_store * rsdst;
  const uint8_t *dst_pad_end = dst + m0 * rsdst;
  __asm__ volatile(
      "sf.vsettnt x0, x0, e8, w1\n"

      "bgeu %[dst], %[dst_store_load_end], 2f\n"
      "beq %[tn_load], %[n0], 1f\n"
      "0:\n"
      "sf.vsettn x0, %[tn_load]\n"
      "sf.vlte8 %[tss_load], (%[src])\n"
      "add %[tss_load], %[tss_load], %[kRowInc]\n"
      "add %[src], %[src], %[rssrc]\n"
      "sf.vsettn x0, %[n0]\n"
      "sf.vste8 %[tss_store], (%[dst])\n"
      ...
      : [dst_store_load_end] "r"(dst_store_load_end),
        ...
        [kRowInc] "rI"(kRowInc), [rsdst] "r"(rsdst * sizeof(uint8_t)),
        [rssrc] "r"(rssrc * sizeof(uint8_t)), [k0] "r"(k0), [k1] "r"(k1),
 ...
 */
SKL_FUNC_PRIVATE void skl_pack_tile_set_e8_xsfmmbase(size_t tm, size_t tn,
                                                     size_t tss, uint8_t pad)
    __riscv_inout("xsfmm") {
  if (tm == 0 || tn == 0) {
    return;
  }

  vuint8m2_t pad_vec = __riscv_vmv_v_x_u8m2(pad, tn);

  const size_t kRowInc = 1;
  size_t tss_end = tss + kRowInc * tm;
  __asm__ volatile("bgeu %[tss], %[tss_end], 1f\n"
                   "sf.vsettnt x0, %[tn], e8, w1\n"
                   "0:\n"
 ...
 */
SKL_FUNC_PRIVATE void skl_pack_padding_optional_e8_e8rcptextec_xsfmmbase(
    size_t m, size_t n, const uint8_t *src, size_t rs, uint8_t *dst, size_t cs0,
    size_t rs1, size_t cs1, bool pad_right, bool pad_bottom, uint8_t pad)
    __riscv_new("xsfmm") {
  if (m == 0 || n == 0) {
    return;
  }
  ...
  size_t ete = 0;
  __asm__ volatile("sf.vsettnt %[ete], x0, e8, w1"
                   : [ete] "=r"(ete)
                   :
                   : "vtype", "vl");
  const size_t m0 = ete;
  ...
  skl_pack_tile_load_e8_e8_xsfmmbase(tm_load, tn_load, src, rs, mt0);
  ...
  for (; i1 + 1 < m1; i1 += 2) {
    ...
    if (i1 == m1 - 2) {
      ...
      skl_pack_tile_set_e8_xsfmmbase(m0_bottom - tm_bottom, n0,
                                     mt4 + tm_bottom * kRowInc, pad);
    }
    for (j1 = 0; j1 + 1 < n1; ++j1) {
      size_t tn_load_right = j1 == n1 - 2 ? tn_right : n0;
      skl_pack_tile_store_load_e8_e8_xsfmmbase(
    ...
    if (i1 < m1 - 2) {
      ...
      skl_pack_tile_store_pad_bottom_e8_e8_xsfmmbase(
...
SKL_FUNC void skl_pack_e8_e8rcptex1c_xsfmmbase(size_t m, size_t n,
                                               const uint8_t *src, size_t rs,
  ...
  skl_pack_padding_optional_e8_e8rcptextec_xsfmmbase(
