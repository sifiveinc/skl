...
/**
 * @brief TE x 1 matrix packing kernel for 8-bit matrices using SiFive's Xsfmm
 * matrix engine.
 *
 ...
 * ```
 *
 * This function is intended to provide packing functionality for all 8-bit
 * datatypes by way of type-punning through the input/output array pointers and
 * of the padding value.
 */
void skl_pack_e8_e8rcptex1c_xsfmmbase(size_t m, size_t n, const uint8_t *src,
                                      size_t rs, uint8_t *dst, size_t rs1,
                                      size_t cs1, uint8_t pad);

/**
 * @brief Xsfmm matrix transposition for 8-bit matrices.
 *
