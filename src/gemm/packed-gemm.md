...
- `A`: `f32rcptex1c` `TE` x 1 column vectors of `A` packed in block-row- or block-column-major order, depending on `rsa1` and `csa1`
- `B`: `f32rcp1xte` 1 x `TE` row vectors of `B` packed in block-row- or block-column-major order, depending on `rsb1` and `csb1`
- `C`: `f32rcptexterc` `TE` x `TE` row- or column-major blocks of `C` packed in block-row- or block-column-major order, depending on `rsc1` and `csc1`
