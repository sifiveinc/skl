...
/**
 * @brief Xsfmm quad-widening OFP8 E5M2 GEMM.
 *
 ...
 *
 * Computes `C = alpha * A * B + beta * C` for OFP8 E5M2 column-major matrix A,
 * OFP8 E5M2 row-major matrix B, and float32 row-major matrix C.
 *
 ...
 * as `csa`.
 **/
void skl_gemm_f8e5m2c_f8e5m2_f32_xsfmm32a8f(size_t m, size_t n, size_t k,
                                            float alpha, const uint8_t *a,
                                            size_t csa, const uint8_t *b,
                                            size_t rsb, float beta, float *c,
                                            size_t rsc);

/**
 * @brief Xsfmm packed quad-widening OFP8 E5M2 GEMM.
 *
 ...
 *
 * Computes `C = alpha * A * B + beta * C` for packed OFP8 E5M2 matrices A and
 * B, and packed float32 matrix C.
 *
 * Equivalent to calling:
 * ```
 ...
 * ```
 */
void skl_gemm_f8e5m2rcptex1c_f8e5m2rcp1xte_f32rcptexterc_xsfmm32a8f(
