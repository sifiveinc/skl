...
/**
 * @brief Xsfmm OFP8 E4M3 A * B matrix-matrix multiplication with float32 output
 ...
 * true`) for OFP8 E4M3 column-major matrix A, OFP8 E4M3 row-major matrix B, and
 * FP32 row-major matrix C. The entries of A and B are type-punned as 8-bit
 * unsigned integers.
 *
 * Equivalent to scalar call:
 * ```
 * skl_gemm_f8e4m3rc_f8e4m3rc_f32rc_scalar(
 *     m, n, k,       // m, n, k
 *     1,             // alpha
 *     a, 1, csa,     // a, rsa, csa
 *     b, rsb, 1,     // b, rsb, csb
 ...
 * as `csa`.
 **/
void skl_gemm_a1b01_f8e4m3c_f8e4m3_f32_xsfmm32a8f(size_t m, size_t n, size_t k,
...
/**
 * @brief Xsfmm OFP8 E4M3 A * B packed matrix-matrix multiplication with float32
 * output.
 *
