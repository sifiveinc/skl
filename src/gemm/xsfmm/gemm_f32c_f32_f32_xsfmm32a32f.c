...
void skl_gemm_tile_load_f32_f32rcptexterc_f32_xsfmmbase(
  ...
  size_t n1 = (n + ete - 1) / ete;
  size_t m_avl = m;
  for (size_t i1 = row1; i1 < row1 + m1; ++i1) {
    size_t tm = m_avl >= ete ? ete : m_avl;
    size_t n_avl = n;
    for (size_t j1 = col1; j1 < col1 + n1; ++j1) {
      size_t tn = n_avl >= ete ? ete : n_avl;
      size_t tss_tile = tss + i1 * rstss + j1 * cstss;
      const float *c_block = (float *)c + i1 * rsc1 + j1 * csc1;
      size_t i0 = 0;
      if (csc0 == 1) {
        __asm__ volatile(
 ...
 */
SKL_FUNC_PRIVATE void
skl_gemm_apply_tiling_f32rcptex1c_f32rcp1xte_rcptexterc_xsfmm32a32f(
    skl_gemm_tile_load_f32_rcptexterc_f32_xsfmmbase_t load_func, size_t m1,
    size_t n1,
  ...
  if (accum) {
    load_func(m, n, c, rsc0, csc0, rsc1, csc1, row1, col1, trans ? mt0c : mt0,
              rstss, cstss);
  } else {
    if (trans) {
 ...
 */
SKL_FUNC_PRIVATE void
skl_gemm_fused_f32rcptex1c_f32rcp1xte_rcptexterc_xsfmm32a32f(
    skl_gemm_tile_load_f32_rcptexterc_f32_xsfmmbase_t load_func, size_t m,
