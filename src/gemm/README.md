...
For example, the Xsfvqdotq GEMM kernels currently require `alpha=1` and `beta=0`.
Others require specific memory layouts, such as packing or transposition.
These constraints are indicated in the function name, as described below (e.g. `skl_gemm_a1b01_i8_i8pc_i32_xsfvqdotq`).
...
- `skl_gemm_f32pc_f32cp_f32rcp_xsfmm32a32f(...)`  (See the [Packed GEMM API](packed-gemm.md) document for details)
...
- `skl_gemm_2tm2tn_f32pc_f32cp_f32rcp_xsfmm32a32f` for a 2TEx2TE register tile (`2*TE` rows, `2*TE` columns)
