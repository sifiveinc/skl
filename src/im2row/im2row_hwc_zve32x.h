...
/**
 * @brief Convert input tensor to im2row matrix with RVV optimization
 *
 * Processes all output positions for a single batch, generating the complete
 * im2row matrix in one call. Uses optimized bulk memory operations exploiting
 * HWC channel-contiguous layout. Automatically selects specialized non-dilated
 * kernel when dilation_width == 1 && dilation_height == 1 for better
 * performance. Supports any primitive data type through void pointers and
 ...
 * performance
 */
void skl_im2row_hwc_zve32x(void *output, const void *input, size_t element_size,
