...
SKL_FUNC_PRIVATE void skl_im2row_single_row_hwc_zve32x(
...
SKL_FUNC void skl_im2row_hwc_zve32x(
  ...
  if (dilation_width == 1 && dilation_height == 1 &&
      input_channel == input_width_stride) {
    for (size_t j = 0; j < output_height; j++) {
      ...
      for (size_t i = 0; i < output_width; i++) {
        ...
        skl_im2row_d1_full_patch_hwc_zve32x(
            ...
            filter_width, input_height_stride, input_width_stride, zero_byte);
    ...
    for (size_t j = 0; j < output_height; j++) {
      ...
      for (size_t i = 0; i < output_width; i++) {
        ...
        skl_im2row_single_row_hwc_zve32x(
            row_tile, input, element_size, in_h_origin, in_w_origin,
