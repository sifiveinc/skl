 ...
 * efficient convolution-to-GEMM transformation for 16-bit element types.
 *
...
/**
 * @brief Convert input tensor to im2row matrix with RVV optimization for 16-bit
 * elements
 *
 * Type-safe version of skl_im2row_hwc_zve32x for 16-bit element types
 * (uint16_t, int16_t, _Float16). Processes all output positions for a single
 ...
 *
 * @note Type-safe wrapper: element_size is fixed at sizeof(uint16_t)
 ...
 * performance
 */
void skl_im2row_hwc_e16_zve32x(uint16_t *output, const uint16_t *input,
