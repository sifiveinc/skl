 ...
 */
SKL_FUNC_PRIVATE vfloat32m8_t skl_sigmoid_leftexp_zve32f_f32m8(vfloat32m8_t x,
                                                               size_t vl) {
...
SKL_FUNC void skl_sigmoid_f32_zve32f(float *out, float beta, const float *x,
  ...
  for (size_t i = 0; i < n; i += vl) {
    ...
    /* exp(-|beta x|) */
    const vfloat32m8_t e = skl_sigmoid_leftexp_zve32f_f32m8(
        __riscv_vfsgnj_vf_f32m8(vx, -1, vl), vl);
