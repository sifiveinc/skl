...
SKL_FUNC_PRIVATE vfloat32m8_t skl_leftexp_f32m8(vfloat32m8_t x, size_t vl);

SKL_FUNC void skl_logistic_3u_f32_zve32f(float *out, const float *in,
                                         size_t n) {
  size_t vl;
  for (size_t i = 0; i < n; i += vl) {
    ...
    const vfloat32m8_t n =
        skl_leftexp_f32m8(__riscv_vfsgnj_vf_f32m8(vx, -1, vl), vl);
