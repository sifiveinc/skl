...
/* Determines the optimal vectorization strategy for 32-bit element transpose.
...
skl_transpose_mvec_x8_e32_zve32x(size_t m, size_t n,
 ...
 */
SKL_FUNC_PRIVATE void
skl_transpose_mvec_x4_e32_zve32x(size_t m, size_t n,
                                 const uint32_t *SKL_RESTRICT a, size_t rsa,
                                 uint32_t *SKL_RESTRICT at, size_t rsat) {
 ...
 */
SKL_FUNC_PRIVATE void
skl_transpose_mvec_x2_e32_zve32x(size_t m, size_t n,
                                 const uint32_t *SKL_RESTRICT a, size_t rsa,
                                 uint32_t *SKL_RESTRICT at, size_t rsat) {
 ...
 */
SKL_FUNC_PRIVATE void
skl_transpose_mvec_x1_e32_zve32x(size_t m, size_t n,
                                 const uint32_t *SKL_RESTRICT a, size_t rsa,
                                 uint32_t *SKL_RESTRICT at, size_t rsat) {

  const ptrdiff_t input_bstride = (ptrdiff_t)(rsa * sizeof(uint32_t));
  ...
  for (size_t ii = 0; ii < n; ii += 1) {
    ...
    for (size_t jj = 0; jj < m; jj += vl) {
      ...
      vuint32m8_t v_data = __riscv_vlse32_v_u32m8(in_tile, input_bstride, vl);
      __riscv_vse32_v_u32m8(out_tile, v_data, vl);
      in_tile += vl * rsa;
      out_tile += vl;
    }
  }
...
skl_transpose_mvec_e32_zve32x(size_t m, size_t n,
  ...
  skl_transpose_mvec_x8_e32_zve32x(m, n_end - n_begin, a + n_begin, rsa,
                                   at + n_begin * rsat, rsat);

  n_begin = n_end;
  n_end = n_begin + ((n - n_begin) / 4) * 4;

  skl_transpose_mvec_x4_e32_zve32x(m, n_end - n_begin, a + n_begin, rsa,
                                   at + n_begin * rsat, rsat);

  n_begin = n_end;
  n_end = n_begin + ((n - n_begin) / 2) * 2;

  skl_transpose_mvec_x2_e32_zve32x(m, n_end - n_begin, a + n_begin, rsa,
                                   at + n_begin * rsat, rsat);

  n_begin = n_end;
  n_end = n;

  skl_transpose_mvec_x1_e32_zve32x(m, n_end - n_begin, a + n_begin, rsa,
                                   at + n_begin * rsat, rsat);
}
 ...
 */
SKL_FUNC_PRIVATE void
skl_transpose_nvec_x8_e32_zve32x(size_t m, size_t n,
                                 const uint32_t *SKL_RESTRICT a, size_t rsa,
                                 uint32_t *SKL_RESTRICT at, size_t rsat) {
  size_t vl = 0;
 ...
 */
SKL_FUNC_PRIVATE void
skl_transpose_nvec_x4_e32_zve32x(size_t m, size_t n,
                                 const uint32_t *SKL_RESTRICT a, size_t rsa,
                                 uint32_t *SKL_RESTRICT at, size_t rsat) {
  size_t vl = 0;
  const ptrdiff_t output_seg_bstride = (ptrdiff_t)(rsat * sizeof(uint32_t));
  ...
  for (size_t ii = 0; ii + 3 < m; ii += 4) {
    for (size_t jj = 0; jj < n; jj += vl) {
      ...
      __riscv_vssseg4e32_v_u32m2x4(at + (jj * rsat) + ii, output_seg_bstride,
                                   vrows, vl);
    }
  }
}
 ...
 */
SKL_FUNC_PRIVATE void
skl_transpose_nvec_x2_e32_zve32x(size_t m, size_t n,
                                 const uint32_t *SKL_RESTRICT a, size_t rsa,
                                 uint32_t *SKL_RESTRICT at, size_t rsat) {
  size_t vl = 0;
  const ptrdiff_t output_seg_bstride = (ptrdiff_t)(rsat * sizeof(uint32_t));
  ...
  for (size_t ii = 0; ii + 1 < m; ii += 2) {
    for (size_t jj = 0; jj < n; jj += vl) {
      ...
      __riscv_vssseg2e32_v_u32m4x2(at + (jj * rsat) + ii, output_seg_bstride,
                                   vrows, vl);
    }
  }
}
 ...
 */
SKL_FUNC_PRIVATE void
skl_transpose_nvec_x1_e32_zve32x(size_t m, size_t n,
                                 const uint32_t *SKL_RESTRICT a, size_t rsa,
                                 uint32_t *SKL_RESTRICT at, size_t rsat) {
  size_t vl = 0;
  const ptrdiff_t output_bstride = (ptrdiff_t)(rsat * sizeof(uint32_t));

  // NOLINTBEGIN(clang-analyzer-deadcode.DeadStores)
  vuint32m8_t vrow0 = __riscv_vundefined_u32m8();
  // NOLINTEND(clang-analyzer-deadcode.DeadStores)

  for (size_t ii = 0; ii < m; ii++) {
    for (size_t jj = 0; jj < n; jj += vl) {
      vl = __riscv_vsetvl_e32m8(n - jj);
      vrow0 = __riscv_vle32_v_u32m8(a + (ii + 0) * rsa + jj, vl);
      __riscv_vsse32_v_u32m8(at + (jj * rsat) + ii, output_bstride, vrow0, vl);
    }
  }
}
...
skl_transpose_nvec_e32_zve32x(size_t m, size_t n,
  ...
  skl_transpose_nvec_x8_e32_zve32x(m_end - m_begin, n, a + m_begin * rsa, rsa,
                                   at + m_begin, rsat);

  m_begin = m_end;
  m_end = m_begin + ((m - m_begin) / 4) * 4;

  skl_transpose_nvec_x4_e32_zve32x(m_end - m_begin, n, a + m_begin * rsa, rsa,
                                   at + m_begin, rsat);

  m_begin = m_end;
  m_end = m_begin + ((m - m_begin) / 2) * 2;

  skl_transpose_nvec_x2_e32_zve32x(m_end - m_begin, n, a + m_begin * rsa, rsa,
                                   at + m_begin, rsat);

  m_begin = m_end;
  m_end = m;

  skl_transpose_nvec_x1_e32_zve32x(m_end - m_begin, n, a + m_begin * rsa, rsa,
                                   at + m_begin, rsat);
}
