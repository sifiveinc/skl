...
SKL_FUNC void skl_transpose_e8_xsfmmbase(size_t m, size_t n,
  ...
  for (size_t i = 0; i < m; i += tm) {
    for (size_t j = 0; j < n; j += tn) {
      ...
      const uint8_t *SKL_RESTRICT a_load = a + i * rsa + j;
      uint8_t *SKL_RESTRICT at_store = at + j * rsat + i;

      size_t k = 0;
      __asm__ volatile(
          "sf.vsettnt %[tn], %[avl_n], e8, w1\n"
          "sf.vsettm %[tm], %[avl_m]\n"
          ...
          : [avl_m] "r"(m - i), [avl_n] "r"(n - j),
            [sa] "r"(rsa * sizeof(uint8_t)), [sat] "r"(rsat * sizeof(uint8_t)),
            [kInc] "rI"(kInc)
          : "vtype", "vl", "memory");
    }
  }
