...
/**
 * @brief Consolidated bf16 sigmoid-family test cases for xsfvfexpa.
 *
...
unary_bf16_t tests[] = {
    ...
    FUNCTION_BENCHMARKS(skl_logistic_bf16_xsfvfexpa, -1, 1),
    FUNCTION_BENCHMARKS(skl_silu_bf16_xsfvfexpa, -1, 1),
    FUNCTION_BENCHMARKS(test_swish_bf16_xsfvfexpa, -1, 1),
    FUNCTION_BENCHMARKS(test_glu_bf16_xsfvfexpa, -1, 1),
    FUNCTION_BENCHMARKS(test_swiglu_bf16_xsfvfexpa, -1, 1),
#endif
#if defined(SKL_ENABLE_TESTS)
    FUNCTION_TESTS(skl_logistic_bf16_xsfvfexpa, skl_logistic_bf16_ref,
                   LOGISTIC_MIN, LOGISTIC_MAX, 3.0f),
    FUNCTION_TESTS(skl_silu_bf16_xsfvfexpa, skl_silu_bf16_ref, SILU_MIN,
                   SILU_MAX, 3.0f),
    FUNCTION_TESTS(test_swish_bf16_xsfvfexpa, ref_swish_bf16, SWISH_MIN,
                   SWISH_MAX, 3.0f),
    FUNCTION_TESTS(test_glu_bf16_xsfvfexpa, ref_glu_bf16, GLU_MIN, GLU_MAX,
                   3.0f),
    FUNCTION_TESTS(test_swiglu_bf16_xsfvfexpa, ref_swiglu_bf16, SWIGLU_MIN,
                   SWIGLU_MAX, 3.0f),
#endif
};

static skl_test_suite_t suite = {.name = "skl_sigmoid_bf16_xsfvfexpa",
