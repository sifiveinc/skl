...
/**
 * @brief Implementation of the gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc test
 * harness.
 *
 * This file defines all harness functions _except_ `skl_test_execute`, which is
 * defined in the test file (e.g.
 * xsfmm/skl_gemm_f8e5m2c_f8e5m2_f32_xsfmm32a8f.c).
 */

#include "gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc.h"
#include "skl-ref.h"
...
void gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_init(skl_test_t *t) {
  gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_t *h =
      (gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_t *)t->harness;
  ...
  // Allocate buffers
  SKL_TEST_BUF_CREATE(t, uint8_t, &h->a_pack);
  SKL_TEST_BUF_CREATE(t, uint8_t, &h->b_pack);
...
void gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_verify(skl_test_t *t) {
  gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_t *h =
      (gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_t *)t->harness;
  ...
  float alpha = h->alpha;
  float beta = h->beta;
  uint8_t *a_pack = h->a_pack.data;
  uint8_t *b_pack = h->b_pack.data;
  ...
  for (size_t i = 0; i < a_pack_len; ++i) {
    a_wide[i] = fabsf(skl_cvt_f8e5m2_f32(a_pack[i]));
  }
  for (size_t i = 0; i < b_pack_len; ++i) {
    b_wide[i] = fabsf(skl_cvt_f8e5m2_f32(b_pack[i]));
  }
...
void gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_test_report(skl_test_t *t) {
  gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_t *h =
      (gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_t *)t->harness;
...
void gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_benchmark_report(skl_test_t *t) {
  gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_t *h =
      (gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_t *)t->harness;

  gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_test_report(t);

  size_t maccs = h->m1 * h->n1 * h->k1 * h->m0 * h->n0 * h->k0;
  gemm_rcprc_rcprc_rcprc_report_perf(t, h->steps.warmup, maccs);
}

void gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_cleanup(skl_test_t *t) {
  gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_t *h =
      (gemm_f8e5m2rcprc_f8e5m2rcprc_f32rcprc_t *)t->harness;
