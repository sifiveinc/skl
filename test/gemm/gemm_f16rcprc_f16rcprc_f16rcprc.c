...
/**
 * @brief Implementation of the gemm_f16rcprc_f16rcprc_f16rcprc test harness.
 *
 * This file defines all harness functions _except_ `skl_test_execute`, which is
 * defined in the test file (e.g. rvv/skl_gemm_f16_f16_f16_zvfh_x390.c).
 */

#include "gemm_f16rcprc_f16rcprc_f16rcprc.h"
#include "skl-ref.h"
...
void gemm_f16rcprc_f16rcprc_f16rcprc_init(skl_test_t *t) {
  ...
  // Allocate buffers
  SKL_TEST_BUF_CREATE(t, _Float16, &h->a_pack);
  SKL_TEST_BUF_CREATE(t, _Float16, &h->b_pack);
  SKL_TEST_BUF_CREATE(t, _Float16, &h->c_pack);
  if (h->steps.verify) {
    h->ctx.a_wide =
...
void gemm_f16rcprc_f16rcprc_f16rcprc_verify(skl_test_t *t) {
  ...
  size_t rsc1 = h->rsc1;
  size_t csc1 = h->csc1;
  _Float16 alpha = h->alpha;
  _Float16 beta = h->beta;
  _Float16 *a_pack = h->a_pack.data;
  _Float16 *b_pack = h->b_pack.data;
  _Float16 *c_pack = h->c_pack.data;
  size_t a_pack_len = h->a_pack.len;
  size_t b_pack_len = h->b_pack.len;
  size_t c_pack_len = h->c_pack.len;
  float *a_wide = h->ctx.a_wide;
  float *b_wide = h->ctx.b_wide;
  _Float16 *ref_c = h->ctx.ref_c;
  float *bound = h->ctx.bound;
  ...
  const int P = 11; // 10 bits of mantissa for _Float16 accumulator
  const float u = ldexpf(1.0f, -P); // Maximum relative roundoff error
  // Compute 2 * ((1 + u)^(K1 * K0 + 2) - 1) by change of base formula:
  const float roundoff_scaling = 2 * expm1f((float)(k1 * k0 + 2) * log1pf(u));
  skl_gemm_f32rcprc_f32rcprc_f32rcprc_ref(
      m0, n0, k0, m1, n1, k1, roundoff_scaling * fabsf((float)alpha), a_wide,
      rsa0, csa0, rsa1, csa1, b_wide, rsb0, csb0, rsb1, csb1,
      roundoff_scaling * fabsf((float)beta), bound, rsc0, csc0, rsc1, csc1);

  // Compute the reference result using h->ctx.ref_c
  skl_gemm_f16rcprc_f16rcprc_f16rcprc_ref(
    ...
    for (size_t j1 = 0; j1 < n1; ++j1) {
      for (size_t i0 = 0; i0 < m0; ++i0) {
        for (size_t j0 = 0; j0 < n0; ++j0) {
          ...
          if (fabsf((float)c_pack[idx] - (float)ref_c[idx]) > bound[idx]) {
            SKL_TEST_LOG(t, SKL_TEST_LOG_ERROR,
                         "result [%zu, %zu, %zu, %zu] (%f) != reference (%f) "
                         "[bound = %f]\n",
                         i1, j1, i0, j0, (float)c_pack[idx], (float)ref_c[idx],
                         bound[idx]);
...
void gemm_f16rcprc_f16rcprc_f16rcprc_benchmark_report(skl_test_t *t) {
  ...
  gemm_f16rcprc_f16rcprc_f16rcprc_test_report(t);
