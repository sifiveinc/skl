...
// The min value for E5M2 is 2^-14, represented by 0.00001.00, which is 4 in
// int8_t
#define SKL_TEST_RAND_MIN_I8 4
// The max value for E5M2 is 1.75 * 2^15, represented by 0.11110.11, which is
// 123 in int8_t
#define SKL_TEST_RAND_MAX_I8 123
...
void skl_gemm_a1b01_f8e5m2c_f8e5m2_f32_xsfmm32a8f_wrapper(
  ...
  skl_gemm_a1b01_f8e5m2c_f8e5m2_f32_xsfmm32a8f(m, n, k, a, csa, b, rsb, c, rsc,
                                               beta != 0.f);
}
#endif
...
int check_error(void) {
  ...
  for (size_t i = 0; i < ALEN; ++i) {
    a_wide[i] = fabs((double)skl_cvt_f8e5m2_f32(a[i]));
  }
  for (size_t i = 0; i < BLEN; ++i) {
    b_wide[i] = fabs((double)skl_cvt_f8e5m2_f32(b[i]));
  }
