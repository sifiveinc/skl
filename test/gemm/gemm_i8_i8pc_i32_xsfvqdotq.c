...
int main(void) {
  ...
  skl_gemm_i8rcp1x4_i8p4x1c_i32_xsfvqdotq(M, N, K, ALPHA, a, (size_t)RSA, K0,
                                          b_pack, (size_t)RSB1, BETA, test_c,
                                          (size_t)RSC);
  res += check_error();
#endif // ENABLE_TEST

  SKL_BENCHMARK_RUN("skl_gemm_i8_i8pc_i32_xsfvqdotq", M * N * K,
                    SKL_TEST_WARMUP, skl_gemm_i8rcp1x4_i8p4x1c_i32_xsfvqdotq, M,
                    N, K, ALPHA, a, (size_t)RSA, K0, b_pack, (size_t)RSB1, BETA,
                    c, (size_t)RSC);
