...
/**
 * @brief Implementation of the transpose_e16 test harness.
 *
 * This file defines all harness functions _except_ `execute`, which is
 * defined in the test file (e.g. rvv/skl_transpose_e16_zve32x.c).
 */
...
void transpose_e16_init(skl_test_t *t) {
  ...
  SKL_TEST_BUF_CREATE(t, uint16_t, &h->a);
  SKL_TEST_BUF_CREATE(t, uint16_t, &h->at);
  if (h->steps.verify && h->at.len) {
    h->ctx.ref_at = malloc(h->at.len * sizeof(uint16_t));

    // Copy original `at` contents into ref to check for clobbered data later.
    memcpy(h->ctx.ref_at, h->at.data, h->at.len * sizeof(uint16_t));
  }
}
...
void transpose_e16_verify(skl_test_t *t) {
  ...
  size_t rsa = h->rsa;
  size_t rsat = h->rsat;
  const uint16_t *a = h->a.data;
  const uint16_t *at = h->at.data;
  uint16_t *ref_at = h->ctx.ref_at;

  // Compute reference value
  skl_transpose_e16_ref(m, n, a, rsa, ref_at, rsat);
    ...
    for (size_t j = 0; j < rsat; ++j) {
      ...
      if (at[idx] != ref_at[idx]) {
        SKL_TEST_LOG(t, SKL_TEST_LOG_ERROR,
                     ...
                     (uint32_t)at[idx], (uint32_t)ref_at[idx],
                     j > m ? " (clobbered)" : "");
