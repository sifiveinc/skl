...
void cvt_f32_f8_verify(skl_test_t *t) {
  ...
  if (h->out_type == F8E4M3) {
    if (h->saturation) {
      skl_cvt_sat_f32_f8e4m3_ref(ref, h->in.data, h->scale, h->len);
    } else {
      skl_cvt_f32_f8e4m3_ref(ref, h->in.data, h->scale, h->len);
    }
  } else { // F8E5M2
    if (h->saturation) {
      skl_cvt_sat_f32_f8e5m2_ref(ref, h->in.data, h->scale, h->len);
    } else {
      skl_cvt_f32_f8e5m2_ref(ref, h->in.data, h->scale, h->len);
    }
  }
